...
#Sets
T = range(0, 8759) #Time period {0,...,672}
...
# Define data
A_temp = pd.DataFrame(df2['A']).to_dict()  # Swept area for each wind turbine size (m²)
A = dict(ChainMap(*A_temp.values()))
Cp_temp = pd.DataFrame(df2['Cp']).to_dict()  # Power coefficient for each wind turbine size
...
WKe_temp = pd.DataFrame(df3['WKe']).to_dict() #Max electrical capacity for each wind turbine size (kW)
WKe = dict(ChainMap(*WKe_temp.values()))
RKe_temp = pd.DataFrame(df3['RKe']).to_dict() #Max electrical capacity for each RFC size (kW)
RKe = dict(ChainMap(*RKe_temp.values()))
HFKt_temp = pd.DataFrame(df3['HFKt']).to_dict() #Max thermal capacity for each H2 furnace size (kW)
...
HFCAP_temp = pd.DataFrame(df5['HFCAP']).to_dict() #CAPEX for each H2 furnace ($/kW)
...
HFOP_temp = pd.DataFrame(df5['HFOP']).to_dict() #OPEX for each H2 furnace size ($/kW)
HFOP = dict(ChainMap(*HFOP_temp.values()))
SOP_temp = pd.DataFrame(df5['SOP']).to_dict() #OPEX for each Solar ($/kW)
...
Etas_temp = pd.DataFrame(df6['Etas']).to_dict() #Efficiency of solar
...
Rmin_temp = pd.DataFrame(df6['Rmin']).to_dict() #Minimum H2 capacity of RFC (kg)
Rmin = dict(ChainMap(*Rmin_temp.values()))
H_E_temp = pd.DataFrame(df6['H_E']).to_dict() #Conversion of H2 to electrical energy (kg H2/kWh)
H_E = dict(ChainMap(*H_E_temp.values()))
E_H_temp = pd.DataFrame(df6['E_H']).to_dict() #Conversion of electrical energy to H2 (kg H2/kWh) 
...
M_temp = pd.DataFrame(df6['M']).to_dict() #Big M constant
M = dict(ChainMap(*M_temp.values()))
H2L_temp = pd.DataFrame(df6['H2L']).to_dict() #Lifetime of H2 storage
...
for c in C:
    ...
    # Create the solver interface and solve the model using gurobi
    ...
    # Writes outputs to text and excel file
    ...
    scalar_data = {
        ...
        'Hydrogen storage units (10kg)': (model.n_h2_storage.value),
        'Hydrogen storage (CAPEX)': (model.h2_storage_cc.value),
        'Solar Capacity (kW)': (model.sce.value),
        'Solar Capital cost (CAPEX': (model.scc.value),
        'solar Operational Cost (OPEX)': (model.soc.value),
        'Solar installed area' : (model.ase.value),
    }

    # Wind
    turbine_data = [
        {'Size': '100 kW', 'Built': model.nwe[1].value, 'Capacity (kW)': (model.cwe[1].value), 'CAPEX': model.wcc[1].value, 'OPEX': model.woc[1].value, 'Total annual cost': model.wcc[1].value + model.woc[1].value},
        {'Size': '500 kW', 'Built': model.nwe[2].value, 'Capacity (kW)': (model.cwe[2].value), 'CAPEX': model.wcc[2].value, 'OPEX': model.woc[2].value, 'Total annual cost': model.wcc[2].value + model.woc[2].value},
        {'Size': '800 kW', 'Built': model.nwe[3].value, 'Capacity (kW)': (model.cwe[3].value), 'CAPEX': model.wcc[3].value, 'OPEX': model.woc[3].value, 'Total annual cost': model.wcc[3].value + model.woc[3].value},
        {'Size': '1500 kW', 'Built': model.nwe[4].value, 'Capacity (kW)': (model.cwe[4].value), 'CAPEX': model.wcc[4].value, 'OPEX': model.woc[4].value, 'Total annual cost': model.wcc[4].value + model.woc[4].value},
    ]
    ...
    # RFC
    rfc_data = [
        {'Size': '1000 kW', 'Built': model.nre[1].value, 'Capacity (kW)': (model.cre[1].value), 'CAPEX': model.rcc[1].value, 'OPEX': model.roc[1].value, 'Total annual cost': model.rcc[1].value + model.roc[1].value},
        {'Size': '20000 kW', 'Built': model.nre[2].value, 'Capacity (kW)': (model.cre[2].value),  'CAPEX': model.rcc[2].value, 'OPEX': model.roc[2].value, 'Total annual cost': model.rcc[2].value + model.roc[2].value},
        {'Size': '100000 kW', 'Built': model.nre[3].value, 'Capacity (kW)': (model.cre[3].value), 'CAPEX': model.rcc[3].value, 'OPEX': model.roc[3].value, 'Total annual cost': model.rcc[3].value + model.roc[3].value},
    ]
    ...
    time_series_data = {
        'Timestep': list(T),
        'State of RFC (SoR), (kg-H2)': [(model.SoR[t].value) for t in T],
        ...
        'Charging load (kWh)' : [model.ch[t].value for t in T],
        'Discharging load (kWh)': [model.dc[t].value for t in T]
    }
