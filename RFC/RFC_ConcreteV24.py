...
def create_RFC_modelV24(T, WS, RS, HFS, W, L, LT, WUe, RUe, HFUt, WKe, RKe, HFKt, WCAP, RCAP, HFCAP, WOP, ROP, HFOP, SCAP, SOP, r, WL, SL, RL, HFL, EtaR, Etas, EtaHF, Rmin, H_E, E_H, H_T, M, roe, A, Cp, S, Co, Ci, SKe, H2CAP, H2L):
    model = pyo.ConcreteModel(name="(RFC_OptimizerV25)")

    #Variables    
    #Costs
    model.tc = pyo.Var(within=pyo.NonNegativeReals) # Total cost of microgrid
    model.cc = pyo.Var(within=pyo.NonNegativeReals) # Discounted annual capital cost for all technologies
    ...
    model.oc = pyo.Var(within=pyo.NonNegativeReals) # Discounted annual operating cost for all technologies
    ...
    # Wind
    model.we = pyo.Var(T, WS, within=pyo.NonNegativeReals) # Generation from wind for time t
    ...
    # Solar
    model.swe = pyo.Var(T, within=pyo.NonNegativeReals) # Generation from solar for time t
    model.nse = pyo.Var(within=pyo.NonNegativeIntegers) # Number of solar units installed
    model.ase = pyo.Var(within=pyo.NonNegativeReals) # Total area of solar panels installed
    model.sce = pyo.Var(within=pyo.NonNegativeReals) # Rated capacity of solar panels
    ...
    model.SoR = pyo.Var(T, within=pyo.NonNegativeReals) # Hydrogen storage
    model.SoR0 = pyo.Var(within=pyo.NonNegativeReals, bounds=(Rmin['Const'], SKe[1]))
    ...
    model.max_h2_storage = pyo.Var(within=pyo.NonNegativeReals) # Maximum hydrogen storage through t
    ...
    model.ch = pyo.Var(T, within=pyo.NonNegativeReals)   # charge flow:
    model.dc = pyo.Var(T, within=pyo.NonNegativeReals)   # discharge flow: 
    ...
    def performance_we1(model, t, ws):
        if ws == 1 and Ci[ws] <= W[t] <= Co[ws]: # cut-in cut-out wind speeds
            return model.we[t, ws] <= (0.5 * Cp[ws] * roe['Const'] * A[ws] * (W[t]) ** 3 / 1000) * model.nwe[ws]
    ...
    def capacity_solar(model):
        return model.ase == model.nse * 10
    model.capacity_solar_con = pyo.Constraint(rule=capacity_solar)

    def capacity_solar_2(model):
        return model.sce == model.ase * 1 * Etas['Const'] 
    model.capacity_solar_2_con = pyo.Constraint(rule=capacity_solar_2)
    ...
    def capacity_check_re2(model, t):
        return sum(model.cre[rs] for rs in RS) >= -model.powlvl[t]
    model.capacity_check_re2con = pyo.Constraint(T, rule=capacity_check_re2)
    
    def capacity_check_re3(model, rs):
        return model.cre[rs] <= RKe[rs]
    model.capcity_check_re3con = pyo.Constraint(RS, rule=capacity_check_re3)
    ...
    def RFC_state(model, t): 
        if t > 0:
            return model.SoR[t] == model.SoR[t-1] + (model.ch[t] * EtaR['Const'] * H_E['Const']) - (model.dc[t] * EtaR['Const'] * E_H['Const']) - (((LT[t]) * (H_T['Const'])) / EtaHF['Const'])
        else:
            return model.SoR[t] == (model.SoR0) + (model.ch[t] * EtaR['Const'] * H_E['Const']) - (model.dc[t] * EtaR['Const'] * E_H['Const']) - (((LT[t]) * (H_T['Const'])) / EtaHF['Const'])
    model.RFC_statecon = pyo.Constraint(T, rule=RFC_state)
    
    def RFC_state2(model):
        SoR_final = max(model.SoR) 
        return model.SoR[SoR_final] == model.SoR0
    ...
    def ch_bigM(model, t):
        return model.ch[t] <= M['Const'] * model.y[t]
    model.ch_bigM_con = pyo.Constraint(T, rule=ch_bigM)

    def dc_bigM(model, t):
        return model.dc[t] <= M['Const'] * (1 - model.y[t])
    model.dc_bigM_con = pyo.Constraint(T, rule=dc_bigM)

    def pow_chdc_link(model, t):
        return model.powlvl[t] == model.ch[t] - model.dc[t]
    model.pow_chdc_link_con = pyo.Constraint(T, rule=pow_chdc_link)
