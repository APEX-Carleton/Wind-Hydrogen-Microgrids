...
def create_RFC_modelV24(T, WS, RS, HFS, W, L, LT, WUe, RUe, HFUt, WKe, RKe, HFKt, WCAP, RCAP, HFCAP, WOP, ROP, HFOP, SCAP, SOP, r, WL, SL, RL, HFL, EtaR, Etas, EtaHF, Rmin, H_E, E_H, H_T, M, Hi, roe, A, Cp, S, Co, Ci, SKe, H2CAP, H2L):
   ...
   # Costs
    ...
    model.nwe = pyo.Var(WS, within=pyo.NonNegativeIntegers) # Number of wind turbines
    ...
    # RFC
    model.nre = pyo.Var(RS, within=pyo.NonNegativeIntegers) # Number of units for RFC
    model.cre = pyo.Var(RS, within=pyo.NonNegativeReals) # Total capacity for RFC
    model.SoR = pyo.Var(T, within=pyo.NonNegativeReals) # State of Charge of RFC (amount of H2 stored (kg))

    # H2 Furnace
    model.hft = pyo.Var(T, within=pyo.NonNegativeReals) # Generation from H2 furnace for time t for thermal
    model.nhft = pyo.Var(within=pyo.NonNegativeIntegers) # Number of units for H2 furnace
    ...
    # Power logic
    model.powlvl = pyo.Var(T) # Tracks if there is a deficit or surplus of energy
    ...
    # Ensure turbine operate within cut-in cut-out limits
    ...
    # Ensure wind output does not exceed rated caapcity
    ...
    # Ensure wind capacity does not exceed maximum allowed capacity
    ...
    # Number of (10kg) H2 storage units 
